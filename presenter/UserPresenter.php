...
class UserPresenter extends GroupBasePresenter
{
    ...
    {
        //if ($this->groupManager->canUserView($resourceId, $this->user->id))
        //{
            $ret = $this->userManager->getGroupList($resourceId);
            if (!$ret)
    ...
    private function _getUserRequest($resourceId, $subResourceId)
    {
        //if ($this->groupManager->canUserView($resourceId, $this->user->id))
        //{
            $ret = $this->userManager->getGroupItem($resourceId, $subResourceId);
            if (!$ret)
                $this->httpResponse->setCode(Response::S404_NOT_FOUND);
            else
            {
    ...
    {
        if ($this->groupManager->canUserEdit($urlId, $this->user->id))
        {
            $ret = $this->userManager->userGroupCreate($urlId, $urlId2);
            if (!$ret)
    ...
    private function _deleteUserRequest($urlId, $urlId2)
    {
        ...
        if ($this->groupManager->canUserEdit($urlId, $this->user->id))
        {
            $ret = $this->userManager->userGroupDelete($urlId, $urlId2);
            if (!$ret)
                $this->httpResponse->setCode(Response::S404_NOT_FOUND);
            else
                $this->httpResponse->setCode(Response::S200_OK);
